...
class Solution:
    ...
    def shortestPathBinaryMatrix(self, grid: List[List[int]]) -> int:
        ...
        # location x, y, path length
        pathQueue = collections.deque([(0, 0, 1)])
    ...
    def ladderLength(self, beginWord: str, endWord: str, wordList: List[str]) -> int:
        ...
        while BFSQueue and otherBFSQueue:
            ...
            BFSQueue = {
                ...
                for curWord in BFSQueue
                if differByOneLetter(curWord, word) and word not in visited
            }
    ...
    def maxAreaOfIsland(self, grid: List[List[int]]) -> int:
        ...
        def DFS(i, j):
            if 0 <= i < m and 0 <= j < n and grid[i][j]:
                # so you do not have to use another grid to keep track of visited nodes
                grid[i][j] = 0
    ...
    def findWords(self, board: List[List[str]], words: List[str]) -> List[str]:
        ...
        def DFS(row: int, col: int, trieNode: TrieNode, path: str):
            if row < 0 or row >= ROWS or col < 0 or col >= COLS or board[row][col] not in trieNode.children:
                return
    ...
    def solveSudoku(self, board: List[List[str]]) -> None:
        ...
        # find next empty cell after board[startI][startJ]
        def getNextEmptyCell(startI: int = -1, startJ: int = -1) -> Tuple[int, int]:
